...
function TopicSummaryCard({
  ...
  useEffect(() => {
    const texts = topic.facts.map(f => f.text);
    if (texts.length === 0) { setLoading(false); return; }
    if (texts.length === 1) {
      // Single alpha — show directly, no LLM
      setSummaryText(texts[0]);
      setLoading(false);
      return;
    }
    // Multiple alphas — fetch LLM summary
    api.post(`/topics/${topic.topic_id}/summary`, { facts: texts })
      .then(res => setSummaryText((res.data?.summary as string) ?? texts[0]))
      .catch(() => setSummaryText(texts[0]))  // fallback to first alpha on error
    ...
    <div style={{
      ...
      <div style={{ padding: '12px 16px' }}>
        {loading ? (
          // Skeleton shimmer
          <div>
            <div style={{ height: 13, width: '95%', background: 'var(--color-background-tertiary)', borderRadius: 4, marginBottom: 7 }} />
            <div style={{ height: 13, width: '80%', background: 'var(--color-background-tertiary)', borderRadius: 4, marginBottom: 7 }} />
            <div style={{ height: 13, width: '60%', background: 'var(--color-background-tertiary)', borderRadius: 4 }} />
          </div>
        ) : summaryText ? (
          <p style={{ fontSize: 14, lineHeight: 1.65, color: 'var(--color-text-primary)', margin: 0 }}>
            {summaryText}
          </p>
        ) : null}
      </div>
