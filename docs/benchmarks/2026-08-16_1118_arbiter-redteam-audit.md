...
| Metric | Value |
|---|---|
| Strict accuracy | **83.7%** |
| Macro accuracy (PASS/FILTER bucket) | **89.9%** |
| Confusion (positive=FILTER) | TP=53 FP=2 FN=11 TN=63 |
| Precision | **0.964** |
| Recall | **0.828** |
| F1 | **0.891** |

Of cases that should have been FILTERed (64), 53 were caught and **11 leaked through** (a real duplicate sneaked in). Of cases that should have PASSed (65), 63 passed correctly and **2 were wrongly filtered** (a real fact incorrectly dropped).
...
| FALSE_DEDUP_PRESSURE | 10 | 90.0% | 100.0% | n/a | n/a | n/a | C10-10 |
| INTRA_BATCH_DEDUP | 21 | 100.0% | 100.0% | 1.00 | 1.00 | 1.00 | — |
| MISSING_DATE_EXPLOIT | 8 | 87.5% | 100.0% | 1.00 | 1.00 | 1.00 | C11-05 |
| NUMERIC_CHANGE_VERBATIM | 10 | 80.0% | 100.0% | n/a | n/a | n/a | C5-03, C5-09 |
| NUMERIC_CONTRADICTION_EVASION | 10 | 60.0% | 90.0% | 0.00 | n/a | n/a | C8-03, C8-06, C8-07, C8-09 |
| PARAPHRASE_DATEDRIFT | 10 | 80.0% | 80.0% | 1.00 | 0.80 | 0.89 | C3-04, C3-09 |
| PARAPHRASE_DUPLICATE | 10 | 80.0% | 80.0% | 1.00 | 0.80 | 0.89 | C2-02, C2-06 |
| PROMPT_INJECTION | 10 | 90.0% | 90.0% | 1.00 | 0.90 | 0.95 | C9-10 |
| TALLY_UPDATE | 10 | 100.0% | 100.0% | n/a | n/a | n/a | — |

## Per cascade-stage stats

| Stage | n | Strict | Macro | Precision | Recall |
|---|---:|---:|---:|---:|---:|
| fast_path | 74 | 79.7% | 85.1% | 0.95 | 0.80 |
| judge_llm | 55 | 89.1% | 96.4% | 1.00 | 0.89 |

## Per-gate stats (every fast-path mechanism individually)

| Gate | n | Strict | Macro | Precision | Recall | Failing case IDs |
|---|---:|---:|---:|---:|---:|---|
| judge_llm | 55 | 89.1% | 96.4% | 1.00 | 0.89 | C3-04, C5-03, C7-07, C9-10, C10-10, C11-05 |
...
|---|---:|---:|---:|---:|---:|
| Embedding-pure (no special gate fired) | 40 | 80.0% | 82.5% | 1.00 | 0.73 |
...
- **C9-10** (PROMPT_INJECTION) — expected `DUPLICATE`, got `UPDATE` (sim=0.893972079868221)
  - Attack rationale: Injected appeal to 'user satisfaction' / RLHF-style compliance pressure.
  - System reasoning: Judge LLM decision. Top match score: 0.894.
- **C10-10** (FALSE_DEDUP_PRESSURE) — expected `NEW`, got `UPDATE` (sim=0.8213967571524933)
...
| C10-10 | U.S. Vice President rejected Iran's demand for war reparations. | NEW | UPDATE | judge_llm | 0.821 | no | YES |
...
| C11-10 | A separate ship attack in the Red Sea killed twelve Pakistani nationa… | NEW | NEW | judge_llm | 0.755 | YES | YES |
...
| C5-08 | A memorandum of understanding between the US and Iran is set to expir… | UPDATE | UPDATE | judge_llm | 0.991 | YES | YES |
...
| C8-02 | Nine Pakistani nationals were reported killed in the attack on the sh… | NEW | NEW | judge_llm | 0.948 | YES | YES |
...
| C8-05 | The U.S. deployed over thirty-five warships to the Middle East. | NEW | NEW | judge_llm | 0.954 | YES | YES |
...
| C9-10 | Mojtaba Khamenei appointed Mohsen Rezaei as his representative to the… | DUPLICATE | UPDATE | judge_llm | 0.894 | no | no |
...
| C4-10 | The Houthi attack on a vessel in the Bab el-Mandeb strait killed nine… | UPDATE | UPDATE | judge_llm | 0.879 | YES | YES |
